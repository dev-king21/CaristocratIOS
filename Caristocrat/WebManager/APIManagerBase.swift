...
class APIManagerBase: NSObject
{
    ...
    func getAuthorizationHeader () -> Dictionary<String,String> {
        ...
        var str = "Bearer "
        str.append(AppStateManager.sharedInstance.userData?.user?.access_token ?? "")
    ...
    func getRequestWith<T : Codable>(route: URL,parameters: Parameters,
        ...
        alamoFireManager.request(route, method: .get, parameters: parameters, headers: getAuthorizationHeader()).responseJSON {
            ...
            guard response.result.error == nil else{
                ...
                print("error in calling post request")
                if errorPopup {
                    if (response.result.error! as NSError).code != 53 {
...
//                Utility.showErrorWith(message: "Internal Server Error")
    ...
    func getRequestWithPagination<T : Codable>(route: URL,parameters: Parameters,
                                     success:@escaping (T) -> Void,
                                     responseType:T.Type,
                                     failure:@escaping DefaultAPIFailureClosure,
                                     errorPopup: Bool,
        ...
        alamoFireManager.request(route, method: .get, parameters: parameters, headers: getAuthorizationHeader()).responseJSON {
            ...
            if response.result.isSuccess {
                if let jsonResponse = response.result.value as? Dictionary<String, AnyObject> {
                    if let parsedResponse =  Utility.decodeJson(dictionary: jsonResponse,  responseType: ResponseWrapper<T>.self), let data = parsedResponse.data {
                        LastService.totalCount = parsedResponse.totalItemCount ?? 0
                        success(data)
                        print(data)
                    }
 ...
 //               Utility.showErrorWith(message: "Internal Server Error")
    ...
    func requestPOSTWithSingleImage<T : Codable> (_ strURL : String, parameters : Parameters,dataImage : [String:Any]? , headers : [String : String]?,  success:@escaping (T) -> Void, responseType:T.Type, failure:@escaping (Error) -> Void){
        //var dictionary = ["Item 1": "description", "Item 2": "description"]
        
        let imageValue = dataImage?["value"] as? UIImage?
        let imageKey = dataImage?["key"] as? String?
        let URLSTR = try! URLRequest(url: strURL, method:.post,headers: getAuthorizationHeader())
        print(URLSTR)
        ...
        Alamofire.upload(multipartFormData: { (multipartFormData) in
            ...
            if(imageValue! != nil) {
                let imageData = imageValue!!.jpegData(compressionQuality: 0.25)
                if let imageData = imageData, let imageKey = imageKey {
...
//                                    }
 ...
 //                                       Utility.showErrorWith(message: "Internal Server Error")
                                    ...
                                    if let jsonResponse = responseObject.result.value as? Dictionary<String, AnyObject> {
                                        if let parsedResponse =  Utility.decodeJson(dictionary: jsonResponse, responseType: ResponseWrapper<T>.self) {
                                            if !(parsedResponse.isSuccess ?? false) {
                                                    if let data = (jsonResponse["data"] as? Dictionary<String, AnyObject>), let errors = data["errors"] as? Dictionary<String, AnyObject>, let key = errors.keys.first, let messages = errors[key] as? [String], let message = messages.first {
                                                        Utility.showErrorWith(message: message)
    ...
    func requestPOSTWithMultipleImages<T : Codable>(_ strURL : String, parameters : Parameters,dataImage : [[String:Any]] , success:@escaping (T) -> Void, failure:@escaping (Error) -> Void){
        Utility.startProgressLoading()
        let URLSTR = try! URLRequest(url: strURL, method:.post,headers:getAuthorizationHeader())
        ...
        Alamofire.upload(multipartFormData: { (multipartFormData) in
            ...
            for image in dataImage {
                var imageValue = image["value"] as! UIImage?
                let imageKey = image["key"] as! String?
                print(imageKey)
                print(imageValue)
                if(imageValue != nil) {
                    let image2 = imageValue?.kf.resize(to: CGSize(width: 150, height: 150))
                    let imageData = image2!.pngData()
                    
                    multipartFormData.append(imageData!, withName: imageKey!, fileName: "image\(count).png", mimeType: "image/png")
                }
