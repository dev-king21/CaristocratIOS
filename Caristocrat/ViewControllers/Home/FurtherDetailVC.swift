...
class FurtherDetailVC: BaseViewController {
    ...
    @IBOutlet weak var viewReportIcon: UIImageView!
    
    var viewReportPermission = false
     var observing = false
...
//        commentsCountsLabel.text = ((Int(commentsCountsLabel.text.spl ?? "0") ?? 0) + 1 ).description
    ...
    func setupViewReportButton() {
        
        //if no url then hide
        let urlString = self.newsModel?.related_car

        if (urlString == nil) || (urlString?.isEmpty ?? true) {
            viewReportButton.isHidden = true
            viewReportIcon.isHidden = true
            return
        }

        
        //user logged
        if AppStateManager.sharedInstance.isUserLoggedIn(), let userId = AppStateManager.sharedInstance.userData?.user?.id {
            
            viewReportButton.isHidden = true
            viewReportIcon.isHidden = true
            
            APIManager.sharedInstance.checkReportPayment(userId: "\(userId)", reportId: "\(self.newsId)", success: { (model: [ReportPaymentCheckModel]) in
                
                
                if model.count > 0 {
                    //show report
                    self.viewReportPermission = true
                    self.viewReportButton.setTitle("View Report", for: .normal)
                } else {
                    self.viewReportPermission = false
                    self.viewReportButton.setTitle("Subscribe to View", for: .normal)
                }
                
                self.viewReportButton.isHidden = false
                self.viewReportIcon.isHidden = false
        ...
        //user not logged in
            ...
            viewReportButton.isHidden = false
            viewReportIcon.isHidden = false
        }
    }
...
//        if AppStateManager.sharedInstance.isUserLoggedIn() {
            ...
            if viewReportPermission {
                //show report
                guard let urlString = self.newsModel?.related_car else { return }
                
                let controller = InAppWebView.instantiate(fromAppStoryboard: .Home)
                controller.customHeader = "REPORT"
                controller.linkString = urlString
                controller.scalesPageToFit = true
                self.navigationController?.pushViewController(controller, animated: true)
                
            } else {
                //show alert
                let subscribeiewController = UIStoryboard(name: "Home", bundle: nil).instantiateViewController(withIdentifier: "ReportSubscribeViewController") as! ReportSubscribeViewController
                subscribeiewController.modalPresentationStyle = .overCurrentContext
                subscribeiewController.modalTransitionStyle = .crossDissolve
                subscribeiewController.newsId = self.newsId
                subscribeiewController.subscribedSuccessfully = { showAlert in
                    self.setupViewReportButton()
                }
                
                self.present(subscribeiewController, animated: true, completion: nil)
            }
            
        }
//        else {
//
//            AlertViewController.showAlert(title: "Require Signin", description: "You need to sign in to your account to subscribe and view report. Do you want to signin?") {
//                let signinController = SignInViewController.instantiate(fromAppStoryboard: .Login)
//                signinController.isGuest = true
//                self.present(UINavigationController(rootViewController: signinController), animated: true, completion: nil)
//            }
