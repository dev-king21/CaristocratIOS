...
//    }
    ...
    @IBInspectable var BoldText: String { // Provide here comma seperated words for hightlight
        get {
            return self.BoldText
        } set {
            let words = newValue.split(separator: ",")
            let attributedText = NSMutableAttributedString.init(string: (self.text?.description)!)
            for item in words {
                if let word = self.text?.description as NSString? {
                    let range = word.range(of: item.description)
                    attributedText.addAttributes([NSAttributedString.Key.font : UIFont(name: FontsType.Medium.rawValue, size: font.pointSize)], range: range)
                }
            }
            self.attributedText = attributedText
        }
